...
$(document).ready(function(e) {
	//genera las tabs
	aguarda=false;
    setInterval(function(){cambio();},500);
	...
	$("#guardar").click(function(e) {
		if(requerido()){
			...
			fechamontaje = $("#fechamontaje").val();
			fechadesmont = $("#fechadesmont").val();
			
			dir = document.getElementById("dirEvento").value;
			noInv = document.getElementById("noinvitados").value;
			tel = document.getElementById("telefonoContacto").value;
			$.ajax({
				url:'scripts/guardaCot.php',
				cache:false,
				async:false,
				data:{
					'usuario':usuario,
...
$("#salon").change(function(e) {
	...
	$.ajax({
		...
		success: function(r){
			if(r){
				document.getElementById("dirEvento").value = r;
			}else{
			}
		}
	});
...
function cambio()
{
	...
	if(por == 1)
	{
		...
		if($(".totalevento").val() != "")
			total += parseInt($(".totalevento").val(), 10);
		if($("#dtotal").val() != "")
			total += parseInt($("#dtotal").val(), 10);
		if($("#gtotal").val() != "")
			total += parseInt($("#gtotal").val(), 10);
		$(".totaleventof").val(total);
	}
	else
	{
		var total = parseInt(0,10);
